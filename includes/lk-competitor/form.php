...
function showCompetitorForm(){
	...
	echo '<select id="competition" name="competition" size="1" class="required">';

	echo '<option value="0" id="0" disabled selected>--- Выберите конкурс ---</option>';
	...
	foreach ($getCurrentCompetitions as $currentCompetition):
			...
			echo '<option value="echo $id">';
				echo $name . " – " . $header . " (Заявки принимаются до $fromDate )";
			echo '</option>'; 

	endforeach;
		echo '</select><br />';

	echo "<div id='containerSpecialties'>";
	echo	'<b>Специальность</b><br />';
	
	echo	'<select id="specialties" name="specialty" size="1" class="required">';
	echo	'<option value="0" disabled="" selected="">Выберите вашу специальность</option>';
		...
		echo '<option value="$id" name="$specialty">';
		echo ($specialty); 
		echo '</option>';
		 endforeach;	
	...
	echo ('<div id="subSpecialtiesContainer" style="display:none">
	...
	<option value="0" disabled selected>Выберите вашу Доп.специальность</option>');
		...
		$getCurrentSubSpecialties = getCurrentSubSpecialties();
		...
		echo '<option style="display:none" value="$id" data-parent-id="<$parent_id>" name="<$subSpecialty">';
		echo ($subSpecialty);
		echo '</option>';
		endforeach;
		echo '</select> <br />';
	...
	echo '<option value="<?php echo $musician["id"] ?>"><?php echo $musician["name"] ?></option>';
...
function gma_application_form_func(){
	...
	// 	"
	// 	<div>Уже зарегистрированы на сайте? Войдите на сайт, чтобы заполнить заявку <br />" . "<form action='/wp-admin'><button type='submit'>Войти</button></form></div>
		
	// 	<div>или перейдите по ссылке, если нет регистрации</div>
